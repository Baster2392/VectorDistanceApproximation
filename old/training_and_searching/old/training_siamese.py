...
def validate(model, criterion, vector_size):
    model.eval()
    x_validate, y_validate = vg.generate_sample_data(100, 0, 1, vector_size, True)
...
def train(model, criterion, optimizer, scheduler, epochs, n_samples,
    ...
    for epoch in range(epochs):
        ...
        x_train, y_train = vg.generate_sample_data(n_samples, 0, 1, model.input_dim)
        ...
        # Print progress
        if epoch % 10 == 0:
            print(f'Id: {model.input_dim}, Hdr: {model.hidden_dim}, Lrn: {model.num_siamese_layers}, Lfcn: {model.num_shared_layers} Epoch [{epoch}/{epochs}], Loss: {loss.item()}, Lr: {optimizer.param_groups[0]["lr"]} Best loss: {min_loss}')
...
def grid_search(criterion, optimizer_obj, scheduler_obj, hidden_dims_r, hidden_dims_fc, num_recurrent_layers_list, num_fc_layers_list, epochs, n_samples, lr, loss_tolerance, device, save_model=True):
    scheduler, out_lr = None, None
    path = CSV_FILE_PATH
    with open(path, mode='a', newline='') as csv_file:
        writer = csv.writer(csv_file)

        for hidden_dim, hidden_dim_fc, num_layers_siamese, num_layers_fc in itertools.product(hidden_dims_r, hidden_dims_fc, num_recurrent_layers_list, num_fc_layers_list):
            model = SiameseNetwork(input_dim, hidden_dim, num_layers_siamese, num_layers_fc)
            ...
            try:
                ...
                if save_model:
                    torch.save(model.state_dict(), f"../saved_models/{model.input_dim}_siamese_{time.time()}.pth")

            validate_value = validate(model, criterion, input_dim)
            writer.writerow([input_dim, hidden_dim // input_dim, hidden_dim, hidden_dim_fc, out_lr, num_layers_siamese, num_layers_fc, epoch, loss, validate_value])


if __name__ == '__main__':
    CSV_FILE_PATH = '../../results/test_siamese.csv'
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    criterion = nn.L1Loss()
    optimizer = optim.Adam
    scheduler = None

    input_dims = [i for i in range(50, 401, 50)]
    for input_dim in input_dims:
        hidden_dims_r = [input_dim * 10, input_dim * 15]
        hidden_dims_fc = [input_dim * 10, input_dim * 15]
        learning_rate = 0.001
        num_recurrent_layers_list = [1, 2]
        num_fc_layers_list = [3, 5]
        for i in range(1):
            ...
            grid_search(criterion, optimizer, scheduler, hidden_dims_r, hidden_dims_fc,
                                      ...
                                      epochs=5000000, n_samples=64, lr=learning_rate, loss_tolerance=0.05, device=device, save_model=False)
