...
CSV_FILE_PATH = '../../saved_results/old/results.csv'
bmodel = None
boptimizer = None
bepoch = 0
bmin_loss = float('inf')
...
def plot_validation_results(y_validate_sorted_np, y_pred_sorted_np, input_dim, string, mean_loss, max_loss, datasize,
                            loss_tolerance):
...
def print_validation_results(mean_loss, max_loss, errors, relative_errors, elapsed_time, x_validate,
                             metric='euclidean'):
    print(
...
def save_model_checkpoint(model, optimizer, epoch, min_loss, path, input_dim, hidden_dim, num_siamese_layers,
                          num_shared_layers):
    state = {
...
def train(model, criterion, optimizer, scheduler, epochs, n_samples, data_size,
          ...
          metric='euclidean', checkpoint_path=None, input_dim=None, hidden_dim=None, num_siamese_layers=None,
          num_shared_layers=None, test_size=1000):
    model.to(device)
    criterion.to(device)
    min_error = float('inf')
    ...
    if checkpoint_path:
        model, optimizer, start_epoch, min_loss = load_model_checkpoint(checkpoint_path, model, optimizer, input_dim,
                                                                        hidden_dim, num_siamese_layers,
                                                                        num_shared_layers)
    ...
    x_validate, y_validate = vg.generate_sample_data_with_multithreading(test_size, -5000, 5000, model.input_dim,
                                                                         metric)
...
def plot_weights_and_biases(model):
    ...
    for i in range(num_layers):
        ...
        plt.scatter(np.full(num_neurons, i), np.arange(num_neurons), c=avg_weights_layer, cmap='coolwarm', marker='s',
                    s=100, alpha=0.5)
...
def objective(trial, num_siamese_layers, num_shared_layers, hidden_dim, input_dim, epochs, loss_tolerance, data_size,
              n_samples, metric, test_size, mode):
    ...
    model, _, _, _, val_loss, _, _, validation_results = train(model, criterion, optimizer, scheduler, epochs=epochs,
                                                               ...
                                                               device=device, data_size=data_size, print_every=100,
                                                               metric=metric, checkpoint_path=mode, input_dim=input_dim,
                                                               hidden_dim=hidden_dim,
                                                               num_siamese_layers=num_siamese_layers,
                                                               num_shared_layers=num_shared_layers, test_size=test_size)
...
def run_experiments(hidden_dims, num_siamese_layers_values, num_shared_layers_values, input_dims, epochs_values,
                    loss_tolerance_values, data_size_values, n_samples_values, test_size_values, metrics, trials=1,
                    mode=None):
    results = []
                                ...
                                for n_samples in n_samples_values:
                                    for test_size in test_size_values:
                                        for hidden_dim in hidden_dims:
                                            ...
                                            study.optimize(
                                                lambda trial: objective(trial, num_siamese_layers, num_shared_layers,
                                                                        hidden_dim, input_dim, epochs, loss_tolerance,
                                                                        data_size, n_samples, metric, test_size, mode),
                                                n_trials=trials)
                                            trial = study.best_trial
                                            min_loss = trial.value
                                            user_attrs = trial.user_attrs
                                            if mode:
                                                save_model_checkpoint(bmodel, boptimizer, bepoch, bmin_loss, mode,
                                                                      input_dim, hidden_dim, num_siamese_layers,
                                                                      num_shared_layers)
                                            ...
                                            results.append((num_siamese_layers, num_shared_layers, input_dim, epochs,
                                                            loss_tolerance, data_size, n_samples, trial_results))
...
def save_experiment_results(results, hidden_dims):
    ...
    for (
    num_siamese_layers, num_shared_layers, input_dim, epochs, loss_tolerance, data_size, n_samples, trials) in results:
...
if __name__ == '__main__':
    ...
    results = run_experiments(hidden_dims, num_siamese_layers_values, num_shared_layers_values, input_dims,
                              epochs_values, loss_tolerance_values, data_size_values, n_samples_values,
                              test_size_values, metrics, 1, checkpoint_dir)
